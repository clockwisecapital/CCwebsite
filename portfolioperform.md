...
Changelog v2.0.0:
- YTD calculations now use weekly data (matches Kwanti methodology)
- Added 3Y Trailing section with annualized returns and full risk metrics
- Enhanced chart data with developer-friendly fields (final_return, chart_title, etc.)
- Sharpe ratio uses arithmetic annualized method (Morningstar standard)
"""
...
class PeriodMetrics:
    ...
    num_months: int  # Sample size for warnings (months for full years; weeks for YTD)

    # Annualized returns (for multi-year periods) - Optional fields at end
    portfolio_annualized_return: Optional[float] = None
...
class ChartData:
    ...
    start_date: str
    end_date: str
    # Enhanced fields for developer convenience
    num_data_points: int = 0
    portfolio_final_return: float = 0.0  # Final cumulative return (e.g., 1.1074 = +110.74%)
    benchmark_final_return: float = 0.0  # Final benchmark cumulative return
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "portfolio_final_return": self.portfolio_final_return,
            "benchmark_final_return": self.benchmark_final_return,
...
class PortfolioAnalyzer:
    ...
    def _build_comparison(
            ...
            for port_name, result in portfolio_results.items():
                if result.cumulative_3y:
                    comparison["cumulative_3y"][port_name] = {
                        ...
                        "annualized_return": result.cumulative_3y.portfolio_annualized_return,
                        ...
                        "up_capture": result.cumulative_3y.portfolio_up_capture,
                        "down_capture": result.cumulative_3y.portfolio_down_capture,
                    }
            # Add benchmark values (same for all portfolios)
            comparison["cumulative_3y"]["_benchmark"] = {
                "return": first_result.cumulative_3y.benchmark_return,
                "annualized_return": first_result.cumulative_3y.benchmark_annualized_return,
                "std_dev": first_result.cumulative_3y.benchmark_std_dev,
                "alpha": first_result.cumulative_3y.benchmark_alpha,
                "beta": first_result.cumulative_3y.benchmark_beta,
                "sharpe": first_result.cumulative_3y.benchmark_sharpe_ratio,
                "max_drawdown": first_result.cumulative_3y.benchmark_max_drawdown,
                "up_capture": first_result.cumulative_3y.benchmark_up_capture,
                "down_capture": first_result.cumulative_3y.benchmark_down_capture,
            }
        ...
        if first_result.chart_data:
            comparison["chart"] = {
                ...
                "benchmark_final_return": first_result.chart_data.benchmark_final_return,
                "start_date": first_result.chart_data.start_date,
                "end_date": first_result.chart_data.end_date,
                "num_data_points": first_result.chart_data.num_data_points,
                "data_frequency": first_result.chart_data.data_frequency,
                "chart_title": first_result.chart_data.chart_title,
                "portfolios": {}
            }
            for port_name, result in portfolio_results.items():
                if result.chart_data:
                    comparison["chart"]["portfolios"][port_name] = {
                        "returns": result.chart_data.portfolio_returns,
                        "final_return": result.chart_data.portfolio_final_return
                    }
    ...
    def _get_weekly_data(
        ...
        """Resample to weekly data (used for YTD/partial year calculations)"""
        period_df = self._get_period_data(df, start_date, end_date)

        period_df['Week'] = period_df['Date'].dt.to_period('W')
        weekly = period_df.groupby('Week').agg({
            'Portfolio_Value': 'last',
            'SPX_Value': 'last',
            'TB_Rate': 'mean'
        }).reset_index()

        weekly['Port_Return'] = weekly['Portfolio_Value'].pct_change(fill_method=None)
        weekly['SPX_Return'] = weekly['SPX_Value'].pct_change(fill_method=None)
        weekly['RF_Period'] = weekly['TB_Rate'] / 100 / 52  # Weekly risk-free rate
        weekly['Port_Excess'] = weekly['Port_Return'] - weekly['RF_Period']
        weekly['SPX_Excess'] = weekly['SPX_Return'] - weekly['RF_Period']
    ...
    def _calculate_period_metrics(
        ...
        Uses weekly data for YTD (partial year) periods, monthly for full years.
        This matches Kwanti's methodology.
        """

        period_df = self._get_period_data(df, start_date, end_date)

        # Determine frequency: use weekly for YTD, monthly for full years
        use_weekly = period_name.upper() == "YTD"

        if use_weekly:
            sampled = self._get_weekly_data(df, start_date, end_date)
            annualize_factor = 52  # Weeks per year
        else:
            sampled = self._get_monthly_data(df, start_date, end_date)
            annualize_factor = 12  # Months per year

        # Returns (cumulative - same regardless of frequency)
        ...
        # Calculate annualized returns for multi-year periods
        start_dt = pd.to_datetime(start_date)
        end_dt = pd.to_datetime(end_date)
        years = (end_dt - start_dt).days / 365.25
        if years >= 1.0:
            port_annualized = (1 + port_return) ** (1 / years) - 1
            spx_annualized = (1 + spx_return) ** (1 / years) - 1
        else:
            port_annualized = None
            spx_annualized = None

        # Risk-free rate (average for period)
        rf_annual = sampled['TB_Rate'].mean() / 100

        # Standard deviation (annualized)
        port_std = sampled['Port_Return'].dropna().std() * np.sqrt(annualize_factor)
        spx_std = sampled['SPX_Return'].dropna().std() * np.sqrt(annualize_factor)

        # Beta - Portfolio vs Benchmark
        returns = sampled[['Port_Excess', 'SPX_Excess']].dropna()
        if len(returns) >= 3:
        ...
        if len(returns) >= 3 and not np.isnan(port_beta):
            ...
            port_alpha = (avg_port_excess - port_beta * avg_spx_excess) * annualize_factor
        else:
            port_alpha = np.nan
        ...
        # Sharpe ratios - Morningstar methodology (arithmetic annualized)
        port_excess_std = sampled['Port_Excess'].dropna().std() * np.sqrt(annualize_factor)
        spx_excess_std = sampled['SPX_Excess'].dropna().std() * np.sqrt(annualize_factor)
        avg_port_excess = sampled['Port_Excess'].dropna().mean()
        avg_spx_excess = sampled['SPX_Excess'].dropna().mean()
        port_sharpe = (avg_port_excess * annualize_factor) / port_excess_std if port_excess_std > 0 else np.nan
        spx_sharpe = (avg_spx_excess * annualize_factor) / spx_excess_std if spx_excess_std > 0 else np.nan

        # Max drawdown (daily - same regardless of frequency)
        ...
        # Capture ratios
        returns = sampled[['Port_Return', 'SPX_Return']].dropna()

        up_periods = returns[returns['SPX_Return'] > 0]
        if len(up_periods) > 0:
            port_up_ret = (1 + up_periods['Port_Return']).prod() - 1
            spx_up_ret = (1 + up_periods['SPX_Return']).prod() - 1
        ...
        down_periods = returns[returns['SPX_Return'] < 0]
        if len(down_periods) > 0:
            port_down_ret = (1 + down_periods['Port_Return']).prod() - 1
            spx_down_ret = (1 + down_periods['SPX_Return']).prod() - 1
        ...
        # Number of periods (for context/warnings)
        num_periods = len(sampled) - 1  # -1 because first period has no return
        ...
        return PeriodMetrics(
            ...
            # Annualized returns (for multi-year periods)
            portfolio_annualized_return=round(port_annualized, 4) if port_annualized is not None else None,
            benchmark_annualized_return=round(spx_annualized, 4) if spx_annualized is not None else None,
            # Portfolio metrics
            ...
            num_months=num_periods  # Note: for YTD this is weeks, not months
        )
    ...
    def _get_methodology(self) -> Dict[str, str]:
        ...
        return {
            ...
            "return_frequency": "Weekly for YTD, Monthly for full years; Daily for Max Drawdown",
            "std_dev_annualization": "Period Std Dev x sqrt(annualize_factor) where factor=52 for weekly, 12 for monthly",
            "alpha_annualization": "Period Alpha x annualize_factor",
            "beta_formula": "Cov(Portfolio Excess, Benchmark Excess) / Var(Benchmark Excess)",
            "sharpe_formula": "(Avg Period Excess × annualize_factor) / (Annualized Std Dev of Excess Returns) - Morningstar arithmetic method",
            "capture_formula": "Compound return in up/down periods divided by benchmark",
    ...
    def _generate_chart_data(
        ...
        # Get last valid (non-NaN) values for final returns
        port_final = df['Port_CumulReturn'].dropna().iloc[-1] if df['Port_CumulReturn'].notna().any() else 0.0
        bench_final = df['SPX_CumulReturn'].dropna().iloc[-1] if df['SPX_CumulReturn'].notna().any() else 0.0

        # Calculate period length for chart title
        start_dt = pd.to_datetime(dates[0])
        end_dt = pd.to_datetime(dates[-1])
        years = (end_dt - start_dt).days / 365.25
        if years >= 2.5:
            period_label = f"{round(years)}-Year"
        elif years >= 1:
            period_label = f"{int(years * 12)}-Month"
        else:
            period_label = "YTD"
        ...
        return ChartData(
            ...
            portfolio_final_return=round(port_final, 4),
            benchmark_final_return=round(bench_final, 4),
            data_frequency="daily",
            chart_title=f"{period_label} Cumulative Returns vs S&P 500 TR"
        )
...
def print_individual_report(result: AnalysisResult, portfolio_display_name: str = None):
    ...
    # ==================== 3Y TRAILING (36 Months) ====================
    if result.cumulative_3y:
        print("\n" + "=" * 70)
        print("3-YEAR TRAILING (36 Months)")
        print("=" * 70)
        ...
        metric_width = 25
        val_width = 18
        print(f"{'Metric':<{metric_width}} {display_name:>{val_width}} {'S&P 500 TR':>{val_width}}")
        print("-" * 70)

        # Returns
        print(f"{'Cumulative Return':<{metric_width}} {fmt_pct(p.portfolio_return, 2):>{val_width}} {fmt_pct(p.benchmark_return, 2):>{val_width}}")
        if p.portfolio_annualized_return is not None:
            print(f"{'Annualized Return':<{metric_width}} {fmt_pct(p.portfolio_annualized_return, 2):>{val_width}} {fmt_pct(p.benchmark_annualized_return, 2):>{val_width}}")

        # Risk metrics
        print(f"{'Std Dev':<{metric_width}} {fmt_pct(p.portfolio_std_dev):>{val_width}} {fmt_pct(p.benchmark_std_dev):>{val_width}}")
        print(f"{'Beta':<{metric_width}} {fmt_num(p.portfolio_beta):>{val_width}} {fmt_num(p.benchmark_beta):>{val_width}}")
        print(f"{'Alpha':<{metric_width}} {fmt_pct(p.portfolio_alpha):>{val_width}} {fmt_pct(p.benchmark_alpha):>{val_width}}")
        print(f"{'Sharpe Ratio':<{metric_width}} {fmt_num(p.portfolio_sharpe_ratio):>{val_width}} {fmt_num(p.benchmark_sharpe_ratio):>{val_width}}")
        print(f"{'Max Drawdown':<{metric_width}} {fmt_pct(p.portfolio_max_drawdown):>{val_width}} {fmt_pct(p.benchmark_max_drawdown):>{val_width}}")
        print(f"{'Up Capture':<{metric_width}} {fmt_num(p.portfolio_up_capture):>{val_width}} {fmt_num(p.benchmark_up_capture):>{val_width}}")
        print(f"{'Down Capture':<{metric_width}} {fmt_num(p.portfolio_down_capture):>{val_width}} {fmt_num(p.benchmark_down_capture):>{val_width}}")
...
def print_comparison_report(multi_result: MultiPortfolioResult):
    ...
    # 3Y Trailing (36 Months) comparison
    if "cumulative_3y" in comparison:
        print("\n" + "-" * 90)
        print("  3-YEAR TRAILING (36 Months)")
        print("-" * 90)

        # Include S&P 500 TR column
        header = f"{'Metric':20}" + "".join([f"{name:>{col_width}}" for name in portfolio_names]) + f"{'S&P 500 TR':>{col_width}}"
        print(header)
        print("-" * 90)

        cum_metrics = [
            ("return", "Cumulative Return", fmt_pct, 2),
            ("annualized_return", "Annualized Return", fmt_pct, 2),
            ("std_dev", "Std Dev", fmt_pct, 1),
            ("beta", "Beta", fmt_num, 2),
            ("alpha", "Alpha", fmt_pct, 1),
            ("sharpe", "Sharpe", fmt_num, 2),
        ...
        benchmark_data = comparison["cumulative_3y"].get("_benchmark", {})
        ...
        for metric_key, display_name, fmt_func, decimals in cum_metrics:
            ...
            for port_name in portfolio_names:
                ...
                row += f"{fmt_func(val, decimals):>{col_width}}"
            # Add benchmark value
            benchmark_val = benchmark_data.get(metric_key)
            row += f"{fmt_func(benchmark_val, decimals):>{col_width}}"
            print(row)
